...
import { getMarketBySymbol } from "./market.js";
import { requireClusterByKey } from "./cluster.js";
export const ASSET_CLASSES = ["crypto", "equity", "etf", "commodity", "fx", "index"];
const SELECT = `
...
`;
export function getAssetBySymbol(db, symbol) {
    return db.prepare(`${SELECT} WHERE a.symbol = ?`).get(symbol);
}
export function requireAssetBySymbol(db, symbol) {
    const row = getAssetBySymbol(db, symbol);
    if (row === undefined)
        throw new JanusError("NOT_FOUND", `no asset ${symbol} in the roster`);
    return row;
}
export function addAsset(db, symbol, cls, clusterKey, notes, now) {
    ...
    db.prepare("INSERT INTO asset (market_id, symbol, class, cluster_id, active, notes, added_at) VALUES (?, ?, ?, ?, 1, ?, ?)").run(market.market_id, symbol, cls, clusterId, notes, now);
    return requireAssetBySymbol(db, symbol);
}
export function listAssets(db, filters) {
    const where = [];
    const args = [];
...
export function updateAsset(db, symbol, patch) {
    ...
    if (sets.length === 0)
        throw new JanusError("VALIDATION", "nothing to update; pass --cluster, --class, or --notes");
    args.push(asset.id);
    db.prepare(`UPDATE asset SET ${sets.join(", ")} WHERE id = ?`).run(...args);
    return requireAssetBySymbol(db, symbol);
}
export function setAssetActive(db, symbol, active) {
    const asset = requireAssetBySymbol(db, symbol);
    db.prepare("UPDATE asset SET active = ? WHERE id = ?").run(active ? 1 : 0, asset.id);
    return requireAssetBySymbol(db, symbol);
}
export function removeAsset(db, symbol) {
    const asset = requireAssetBySymbol(db, symbol);
    ...
    const { n } = db
        ...
        .get(asset.id);
    if (n > 0) {
        throw new JanusError("VALIDATION", `${asset.symbol} has ${n} trade${n === 1 ? "" : "s"}; deactivate it instead of removing it`);
    }
 ...
 */
export function requireSymbols(db, symbols) {
 ...
 */
export function eligibleAssets(db) {
    return db
        .prepare(`${SELECT}

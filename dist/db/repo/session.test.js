...
import { ensureSession, getSession, requireSession, resolveSession, readSessionDate, listSessions, stampPhase } from "./session.js";
...
test("a new session has every phase timestamp null", () => {
    ...
    assert.deepEqual([s.macro_at, s.cluster_at, s.coverage_at, s.screen_at, s.score_at], [null, null, null, null, null]);
    db.close();
});
...
test("requireSession throws SESSION_MISSING for an unknown date", () => {
    const db = fresh();
    assert.throws(() => requireSession(db, "1999-01-01"), (e) => e.code === "SESSION_MISSING");
    db.close();
});
...
test("resolveSession with --date requires the session to exist already", () => {
    const db = fresh();
    assert.throws(() => resolveSession(db, "2026-01-01", NOW), (e) => e.code === "SESSION_MISSING");
...
test("readSessionDate with --date still requires the session to exist", () => {
    const db = fresh();
    assert.throws(() => readSessionDate(db, "1999-01-01", NOW), (e) => e.code === "SESSION_MISSING");
    db.close();
});

...
test("recordClusterRead is keyed per cluster and overwrites on re-run", () => {
    ...
    recordClusterRead(db, DATE, c.id, { metrics: { bias: 1.0, judgement: "constructive" }, results: { tilt: 1.0, aligned: 1 } }, NOW);
    recordClusterRead(db, DATE, c.id, { metrics: { bias: -1.0, judgement: "rolling over" }, results: { tilt: -1.0, aligned: 0 } }, NOW);
    const reads = listClusterReads(db, DATE);
    assert.equal(reads.length, 1);
    assert.deepEqual(reads[0].metrics, { bias: -1.0, judgement: "rolling over" }, "text and numeric metrics both round-trip, and stale ones do not survive");
...
test("metrics and results cascade away with the read they belong to", () => {
    ...
    const c = addCluster(db, "majors", "Majors", null, NOW);
    recordMacro(db, DATE, macro, NOW);
    recordClusterRead(db, DATE, c.id, { metrics: { bias: 1, judgement: "constructive" }, results: { tilt: 1, aligned: 1 } }, NOW);

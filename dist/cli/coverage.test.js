...
import { listCoverage } from "../db/repo/coverage.js";
import { nextPhase, todayNY } from "../domain/session.js";
import { handle } from "./coverage.js";
...
const DATE = todayNY();
const fixture = (name) => readFileSync(new URL(`../../test/fixtures/${name}.json`, import.meta.url));
/**
 ...
 */
function startFakeLighter(noBarsFor = []) {
    const server = http.createServer((req, res) => {
        ...
        const body = path === "/api/v1/candles"
...
async function withHarness(run, noBarsFor = []) {
...
test("a scoped run writes its row but leaves the phase incomplete", async () => {
    await withHarness(async (file) => {
        const result = (await handle("run", ["--asset", "XPL"]));
...
test("an unknown symbol fails the whole call and writes nothing", async () => {
    await withHarness(async (file) => {
        await assert.rejects(() => handle("run", ["--asset", "NOSUCH,XPL"]), (e) => e.code === "VALIDATION" && /NOSUCH/.test(e.message));
        const db = openDb(file);
        const rows = listCoverage(db, DATE);
        db.close();
...
test("--asset BTC,BTC-style duplicates are deduped to a single covered row", async () => {
    await withHarness(async () => {
        const result = (await handle("run", ["--asset", "XPL,XPL"]));
        assert.equal(result.covered, 1);
    });
});
...
test("coverage list --asset uppercases the symbol", async () => {
    await withHarness(async () => {
        await handle("run", []);
        const result = (await handle("list", ["--asset", "xpl"]));
...
test("coverage list --asset rejects an unknown symbol", async () => {
    await withHarness(async () => {
        await handle("run", []);
        await assert.rejects(() => handle("list", ["--asset", "NOSUCH,XPL"]), (e) => e.code === "VALIDATION" && /NOSUCH/.test(e.message));
    });
});

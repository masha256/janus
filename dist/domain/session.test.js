...
import { todayNY, nextPhase, assertPhaseOrder, phaseColumn, PHASES } from "./session.js";
const session = (over = {}) => ({
...
test("assertPhaseOrder rejects a phase with an incomplete prerequisite", () => {
    assert.throws(() => assertPhaseOrder(session(), "screen", false), (e) => e.code === "PHASE_ORDER" && /macro/.test(e.message));
});
...
test("everything downstream of coverage still waits for it", () => {
    ...
    assert.throws(() => assertPhaseOrder(session({ macro_at: "x", cluster_at: "x" }), "screen", false), (e) => e.code === "PHASE_ORDER" && /coverage/.test(e.message));
    // Coverage done, no reads: screening still wants the top-down context.
    assert.throws(() => assertPhaseOrder(session({ coverage_at: "x" }), "screen", false), (e) => e.code === "PHASE_ORDER" && /macro/.test(e.message));

...
test("direction is the weighted mean of factors normalised by total |weight|", () => {
    ...
    assert.ok(Math.abs(got.results["weighted_sum"] - 0.65) < 1e-12);
...
test("confidence is the scoring metric's own 0..1 quality, not inherited from the screen", () => {
    const highConf = deriveScore(m(2, 1, 1, 50, false, false, 1), ctx(0, null), DEFAULT_PARAMS);
    const lowConf = deriveScore(m(2, 1, 1, 50, false, false, 0.3), ctx(0, null), DEFAULT_PARAMS);
...
test("agreement is snapshotted: 1 when all aligned, less when factors cancel", () => {
    ...
    const mixed = deriveScore(m(2, -1, 0, 50, false, false), flat, DEFAULT_PARAMS);
...
test("a quiet second day still INITIATES when trend and sentiment persist", () => {
    ...
    const got = deriveScore(m(1.0, 2, 0.5, 30, false, false), ctxWithPosition({ side: null, units: 0 }, coverage(), day1), DEFAULT_PARAMS);
...
test("an entry whose sized risk would breach the book heat limit stands aside", () => {
    const c = { ...contextFlat(), current_heat: 9900 };
    const { plan } = deriveScore(initiateMetrics, c, PARAMS);
    assert.equal(plan.heat_gate, "blocked");
...
test("late_trend fires off the score's crowding factor", () => {
    const got = deriveScore(m(2, 2, 1, 88, false, false), ctxWithPosition({ side: null, units: 0 }, extended()), DEFAULT_PARAMS);
...
test("crowding on the screen row cannot drive late_trend", () => {
    ...
    const got = deriveScore(m(2, 2, 1, 50, false, false), { ...base, screen: { ...base.screen, metrics: { score: 5, crowding: 99 } } }, DEFAULT_PARAMS);
    assert.equal(got.plan.trend_gate, "pass");
});

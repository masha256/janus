...
const f = (catalyst, sentiment, trend, secular) => ({
...
const ctx = (macroTilt, clusterTilt) => ({
    ...flat,
...
test("deriveScore rejects a metric it cannot average", () => {
    ...
    assert.throws(() => deriveScore({ catalyst: "strong" }, flat, DEFAULT_PARAMS), /catalyst must be a number/);
});
...
test("the context is reported as alignment, and does not move strength", () => {
    ...
    assert.deepEqual({ macro: against.results["macro_aligned"], cluster: against.results["cluster_aligned"] }, { macro: 0, cluster: 0 });
    assert.deepEqual({ macro: behind.results["macro_aligned"], cluster: behind.results["cluster_aligned"] }, { macro: 1, cluster: 1 });
});

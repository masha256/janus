...
test("short late_trend fires on the fear tail of crowding", () => {
  ...
  const got = deriveScore(
    m(-2, -2, -1, 10, false, false),
    ctxWithPosition({ side: null, units: 0 }, extendedDown()),
    DEFAULT_PARAMS,
  );
...
function priorWithCatalyst(catalystEffective: number, session_date: string): import("./score.ts").ScoreResult {
  return {
...
test("catalyst decays from the prior effective value when today records nothing new", () => {
  ...
  const got = deriveScore(
    m(0, 1, 0, 50, false, false),
    ctxWithPosition({ side: null, units: 0 }, coverage(), priorWithCatalyst(1.5, "2026-07-30")),
    DEFAULT_PARAMS,
  );
...
test("catalyst decay is per calendar day and bottoms at zero", () => {
  const got = deriveScore(
    m(0, 1, 0, 50, false, false),
    ctxWithPosition({ side: null, units: 0 }, coverage(), priorWithCatalyst(1.0, "2026-07-27")),
    DEFAULT_PARAMS,
  );
...
test("a fresh catalyst beats the carried one when larger, and always when it points the other way", () => {
  const bigger = deriveScore(
    m(2.0, 1, 0, 50, false, false),
    ctxWithPosition({ side: null, units: 0 }, coverage(), priorWithCatalyst(1.5, "2026-07-30")),
    DEFAULT_PARAMS,
  );
  assert.equal(bigger.results["catalyst_effective"], 2.0);
  const flipped = deriveScore(
    m(-0.5, 1, 0, 50, false, false),
    ctxWithPosition({ side: null, units: 0 }, coverage(), priorWithCatalyst(1.5, "2026-07-30")),
    DEFAULT_PARAMS,
  );
...
test("a legacy prior without catalyst_effective carries nothing", () => {
  ...
  const got = deriveScore(
    m(0, 1, 0, 50, false, false),
    ctxWithPosition({ side: null, units: 0 }, coverage(), prior),
    DEFAULT_PARAMS,
  );
  assert.equal(got.results["catalyst_effective"], 0);
});

...
const barDate = (b: Bar): string => new Date(b.t).toISOString().slice(0, 10);
 ...
 */
export function backfillInputs(
  bars: Bar[],
  sessionDate: string,
): { bars: Bar[]; snapshot: Snapshot } {
  const upTo = bars.filter((b) => barDate(b) <= sessionDate);
  const last = upTo.at(-1);
  if (last === undefined) {
    throw new JanusError(
      "INSUFFICIENT_HISTORY",
      `no daily bar at or before ${sessionDate}; the window fetched starts later`,
    );
  }

...
test("screening snapshots the threshold: a later retune does not rewrite history", async () => {
  await withHarness(async (file) => {
    ...
    const first = (await handle("record", ["BTC", "--metric", "score=8", "--metric", "confidence=0.9"])) as {
    ...
    const second = (await handle("record", ["BTC", "--metric", "score=8", "--metric", "confidence=0.9"])) as {
      results: Record<string, number>;
      flagged: boolean;
    };
...
test("cluster override reaches the flag decision", async () => {
  await withHarness(async (file) => {
    ...
    const btc = (await handle("record", ["BTC", "--metric", "score=8", "--metric", "confidence=0.9"])) as {
      results: Record<string, number>;
      flagged: boolean;
    };
    ...
    const eth = (await handle("record", ["ETH", "--metric", "score=8", "--metric", "confidence=0.9"])) as {
      results: Record<string, number>;
      flagged: boolean;
    };
...
test("screen_score equal to the threshold flags (inclusive boundary)", async () => {
  await withHarness(async (file) => {
    ...
    const result = (await handle("record", ["BTC", "--metric", "score=8", "--metric", "confidence=0.5"])) as {
      results: Record<string, number>;
      flagged: boolean;
    };

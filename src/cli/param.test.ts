...
test("param list shows defaults until a global param overrides one", async () => {
  await withHarness(async () => {
    const before = (await handle("list", [])) as { global: Record<string, number>; resolved: Record<string, number> };
...
test("param set rejects a non-numeric value", async () => {
  await withHarness(async () => {
    ...
    await assert.rejects(
      () => handle("set", ["w_catalyst", "NaN"]),
      (e: Error & { code?: string }) => e.code === "VALIDATION" && /must be a number/.test(e.message),
    );
    await assert.rejects(
      () => handle("set", ["w_catalyst", "abc"]),

...
test("listScreen can return only the flagged rows", () => {
  ...
  recordScreen(db, DATE, requireAssetBySymbol(db, "BTC").id,
    { flagged: true, rationale: null, metrics: { score: 5, confidence: 0.5 }, results: { screen_score: 2.5, threshold: 4 } }, NOW);
  recordScreen(db, DATE, requireAssetBySymbol(db, "ETH").id,
    { flagged: false, rationale: null, metrics: { score: 1, confidence: 0.5 }, results: { screen_score: 0.5, threshold: 4 } }, NOW);

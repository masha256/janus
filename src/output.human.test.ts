...
test("scalars render as key: value lines", () => {
  assert.equal(
    renderHuman({ session_date: "2026-08-01", count: 2, phase_complete: true }),
...
test("a metric bag collapses to k=v, and null reads as a dash", () => {
  assert.equal(
    renderHuman({ metrics: { score: 1.5, judgement: "thin" }, rationale: null }),
    "metrics: score=1.5 judgement=thin\nrationale: -",
  );
});
...
test("a wide nested object breaks onto its own indented block", () => {
  ...
  assert.equal(
    renderHuman({ phases: { macro: "t1", cluster_read: null, coverage: null, screen: null, score: null } }),
    ["phases:", "  macro: t1", "  cluster_read: -", "  coverage: -", "  screen: -", "  score: -"].join("\n"),
  );
});
